...
//
// Local, bundled list of Ethereum-family addresses that appear on OFAC's
// Specially Designated Nationals (SDN) list -- addresses the US Treasury
// has publicly identified in sanctions actions (mostly tied to known theft,
// hacking, and money-laundering cases, e.g. Tornado Cash, Lazarus Group).
//
// This check runs entirely on-device against this bundled list. Nothing is
// sent anywhere -- no address, no lookup, no network call -- so this does
// NOT change the "no data leaves your device" privacy posture of the rest
// of the wallet. It's the same client-side screening approach used by
// Uniswap's own web app and most reputable wallet/DEX front-ends.
//
// WHAT THIS IS: a defensible, honest layer that blocks known-bad addresses
// that regulators have already publicly named.
// WHAT THIS IS NOT: real identity verification, and not a guarantee that
// funds are "clean" -- a non-custodial wallet has no way to know where
// crypto came from before it reaches a wallet the user already controls.
// Freshly stolen funds not yet added to a public list will not be caught.
//
// Source: OFAC SDN list, via the community-maintained mirror at
//   https://github.com/0xB10C/ofac-sanctioned-digital-currency-addresses
// (that project re-parses the official OFAC SDN list on a schedule).
// Last refreshed into this file: 2026-09-07. This list goes stale --
// re-pull it periodically from the source above or from OFAC directly
// (https://ofac.treasury.gov) if you maintain this project long-term.
...
const SANCTIONED_SET = new Set(SANCTIONED_ADDRESSES.map((a) => a.toLowerCase()));

function isSanctionedAddress(address) {
...
if (typeof self !== "undefined") {
  self.TM_SANCTIONS = {
    isSanctionedAddress,
    count: SANCTIONED_ADDRESSES.length,
