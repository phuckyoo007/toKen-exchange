...
- Optional accounts (website only): a username + password account syncs just
  three display settings across devices -- currency, language and watchlist.
  It never stores wallet keys, addresses, balances or the recovery phrase, so
  losing an account (or a breach of this server) can never cost anyone their
  funds. Off by default; turn on with `ACCOUNTS_ENABLED=1` once a persistent
  volume is mounted for the SQLite file (see accounts-api.js for details).
  Not available in the Chrome extension yet (different origin, no shared
  session) -- the entry point stays hidden there automatically.
